...
moveFiles = function() {
    unlink("inventories/docs/site_libs", recursive=T)
    file.rename("inventories/site_libs", "inventories/docs/site_libs")
    for (f in list.files(path="inventories/", pattern=".html")) {
        file.remove(paste0("inventories/docs/",f))
        file.rename(paste0("inventories/",f), paste0("inventories/docs/",f))
    }
}
...
setwd("~/github/castoi/inventories")
