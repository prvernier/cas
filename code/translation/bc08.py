...
con = psycopg2.connect(port="5433", database="cas", user="postgres", password="postgres")
#y = gpd.read_postgis("select * from fri.bc08 limit 10000", con, geom_col='wkb_geometry')
#print(y.head())
rows = con.cursor()
rows.execute("SELECT * INTO fri.bc08rnd FROM fri.bc08 ORDER BY random() LIMIT 10000")
rows.execute("SELECT inventory_standard_cd, crown_closure FROM fri.bc08rnd LIMIT 10")
con.commit()
#print(rows.fetchall())

print("standard", "crown_closure")
print("--------", "--------")
for row in rows:
     print(row[0], row[1])
