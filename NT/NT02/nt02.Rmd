...
# nt02 = st_read(con, query="SELECT * from rawfri.nt02;")
# nt02 = mutate(nt02, shape_area=st_area(nt02) ) #, shape_length=st_length(nt02))
# x2 = as_tibble(nt02)
# dbDisconnect(con)
```
...
x = mutate(x, orig_stand_id = fc_id)
```

### RAWFRI

```{r echo=FALSE, message=FALSE, warning=FALSE}  
dfSummary(x["fc_id"], graph.col=FALSE, max.distinct.values=99)
```
...
x = mutate(x, stand_structure=case_when(
    ...
    TRUE ~ structur #mapvalues(structur, friList, casList)
    ))
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}  
x = mutate(x, casfri_area= round(shape_area,1))
```
...
# the following indicates that soil_moist_reg is not applicable (NOT_APPLICABLE)
x$moisture[!x$nat_non_veg %in% errorList & !x$non_for_anth %in% errorList & !x$non_for_veg %in% errorList]
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
# the following indicates that structure_per is not applicable (-8887)
x$strc_per[!x$nat_non_veg %in% errorList & !x$non_for_anth %in% errorList & !x$non_for_veg %in% errorList]
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
# the following indicates that structure_per is not applicable (-8887)
x$crownclos[!x$nat_non_veg %in% errorList & !x$non_for_anth %in% errorList & !x$non_for_veg %in% errorList]
```
...
```{r echo=TRUE, message=FALSE, warning=FALSE}  
# the following indicates that structure_per is not applicable (-8887)
x$height[!x$nat_non_veg %in% errorList & !x$non_for_anth %in% errorList & !x$non_for_veg %in% errorList]
```
...
# DST Attributes
  ...
  - only dist_type_, dist_year_1, dist_ext_upper_1, and dist_ext_lower_1 are applicable
