...
sppList = read_csv("../data/NB_0001/nb_nbi01_species.csv")
...
y = mutate(y,
    ...
    lyr==1, 
      case_when(
        (crown_closure_lower %in% c(-8888,-9998) | height_lower==c(-8888,-9999)) | (fst==0 & (!str_trim(l1trt)=="" | !l1trt=="CC")) ~ "PP",
        #(crown_closure_lower %in% c(-8888,-9998) | height_lower==c(-8888,-9999)) | (fst==0 & !str_trim(l1trt)=="" & !l1trt=="CC") ~ "PP",
        TRUE ~ "PF"),
      case_when(
...
```{r echo=FALSE, message=FALSE, warning=FALSE}  
dfSummary(y$l1estyr[y$lyr==1], graph.col=FALSE, max.distinct.values=999)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}  
dfSummary(y$l1estyr[y$lyr==2], graph.col=FALSE, max.distinct.values=999)
```
