package ModulesV4::SK_conversion06UTM;


use Exporter;
our @ISA = qw(Exporter);
our @EXPORT = qw(&SKUTMinv_to_CAS );
...
{
	my %MoistRegList = ("VR", 1, "VRR", 1, "R", 1, "RW", 1, "W", 1, "WMW", 1, "MW", 1, "MWD", 1,"MWI", 1, "I", 1, "IP", 1, "P", 1, "PD", 1,"PVP", 1, "VP", 1, "VD", 1, "D", 1, "MF", 1, "F", 1, "VF", 1, "MM", 1, "M", 1, "VM", 1, "MW", 1, "W", 1, "VW", 1);
	my $SoilMoistureReg;
...
sub CCUpper 
{
	...
	{	
		if (($Density eq "a") || ($Density eq "A"))            { $CCHigh = 30; }
...
sub CCLower 
{
	...
	elsif ($INV_version eq "UTM")
	{
		...
		if (($Density eq "a") || ($Density eq "A"))            { $CCLow = 6; }
...
sub Species_UTM
{
	...
	$Sp1  =~ s/\s//g; $Sp2  =~ s/\s//g; $Sp3  =~ s/\s//g; $Sp4  =~ s/\s//g; $Sp5  =~ s/\s//g;

	if(!isempty($Sp1) ){$nnsp1++;$notnull++;}
	if(!isempty($Sp2) ){$nnsp1++;$notnull++;}
	if(!isempty($Sp3) ){$nnsp1++;$notnull++;}
	if(!isempty($Sp4) ){$nnsp2++;$notnull++;}
	if(!isempty($Sp5) ){$nnsp2++;$notnull++;}
...
sub NaturallyNonVegetated
{
	...
	if  ($NatNonVeg eq "0" || isempty($NatNonVeg))	{ $NatNonVeg = MISSCODE; }
...
sub NonForested
{
	...
	if  ($NonForVeg eq "0" || isempty($NonForVeg))	{ $NonForVeg =MISSCODE; }
...
sub Disturbance
{
	...
	my %ModList = ("CO", 1, "BO", 1, "SCO", 1, "WCO", 1, "OCO", 1, "OPC", 1, "WPC", 1, "SPC", 1, "WI", 1, "HA", 1, "IN", 1, "DI", 1, "AK", 1, "SL", 1,
	...
	{ 
		...
		{ 
			$Disturbance = MISSCODE.",-1111". "," . $Extent;; 
		}
	} 
	elsif (isempty($ModCode))  
	{
		$Mod = MISSCODE;  $Disturbance = $Mod . "," . $ModYr. "," . $Extent; 
	}
	else 
	{ 
		$Mod = ERRCODE; $Disturbance = $Mod . "," . $ModYr. "," . $Extent;  
	}
...
sub SKUTMinv_to_CAS 
{
	...
	$csv->column_names ($csv->getline ($SKinv));
   	...
   	{	
		...
		{ 
			if (isempty($row->{SP10}) &&  ($row->{HGT} eq "0" ||isempty($row->{HGT}))) 
			{
				$HeightHigh = MISSCODE;
				$HeightLow = MISSCODE;
			}
		...
		if($SpeciesComp =~ m/^#/)
		{
			$keys = "error translating ".$SpeciesComp."#original#".$row->{SP10}.",".$row->{SP11}.",".$row->{SP12}.",".$row->{SP20}.",".$row->{SP21};
			$herror{$keys}++; 
			$Wetland = MISSCODE;
		}
		elsif($SpeciesComp =~ m/^!/)
		{
			$keys = "undefined config ".$SpeciesComp."#original#".$row->{SP10}.",".$row->{SP11}.",".$row->{SP12}.",".$row->{SP20}.",".$row->{SP21};
			$herror{$keys}++; 
			$Wetland = MISSCODE;
		}
		else 
		{
			...
			if($errspec==1)
			{
				$keys = "species, SA= $row->{SA}#".$SpeciesComp."#original#".$row->{SP10}.",".$row->{SP11}.",".$row->{SP12}.",".$row->{SP20}.",".$row->{SP21};
				$herror{$keys}++; 
			}
	  	...
	  	{ 
			if (isempty($row->{SP10}) &&  ($row->{YOO} eq "0" || isempty($row->{YOO}) ))
			{
				$OriginHigh = MISSCODE;
				$OriginLow = MISSCODE;
			}
			elsif ($row->{YOO} ne "0" && !isempty($row->{YOO})) 
			{
				$keys = "Origin"."#".$row->{YOO};
				$herror{$keys}++;
			}
	  	}
 	  	if( ($OriginHigh< $OriginLow) || $OriginHigh>2014 || ($OriginHigh <1700 && $OriginHigh >0) || ($OriginLow <1700 && $OriginLow >0) || $OriginLow>2014) 
 	  	{ 
        ...
        my ($Sp11, @Spreste) = split(",", $SpeciesComp); 
		#if ($Sp11 eq "XXXX MISS") 
		if(isempty($row->{SP10}))
		{
		...
		{ 
	  	...
	  	if (($StandStructureCode eq "S")) 
	  	{
            ...
            #forested
		    if (!isempty($row->{SP10}) || $forested_poly == 1) 
		    {
		      	$LYR_Record1 = $row->{CAS_ID} . "," . $SMR  . "," . MISSCODE . ",1,1";
		      	$LYR_Record2 = $CCHigh . "," . $CCLow . "," . $HeightHigh . "," . $HeightLow . "," . $ProdFor.",".  $SpeciesComp;  # before speciescomp  . "," . $UnProdFor
		...
		if($nbpr ==1 || $nbpr ==0)
		{
			if(isempty($row->{SP10})  && isempty($row->{DIST})  &&  isempty($row->{TYPE}) && $Wetland eq MISSCODE) 
			{	
				$keys = "WILL PROBABLY DROP THIS>>>code for wetland DRAIN+=".$row->{DRAIN};
	 			$herror{$keys}++; 		
			}
			elsif(isempty($row->{SP10})  && isempty($row->{DIST}) && isempty($row->{TYPE})) 
			{
				$keys = "WILL DROP instead of wetland".$row->{DRAIN};
	 			$herror{$keys}++; 
			}
			elsif(isempty($row->{SP10})  && isempty($row->{DIST}) && $row->{TYPE} eq "0")
			{
				$keys = "WILL DROP null TYPE, mlevel =".$row->{MLEVEL};
	 			$herror{$keys}++; 
			}
			elsif(isempty($row->{SP10})) {
				$keys = "WILL DROP instead nfl and dist-TYPE=".$row->{TYPE}."-distcode=".$row->{DIST};
	 			$herror{$keys}++; 
			}
			else 
			{
				$keys ="!!! record may be dropped#".$CAS_ID."bcse>>>specs=".$row->{SP10}."-distcode=".$row->{DIST}."-NPcode=".$row->{TYPE}."-wetcode=".$row->{DRAIN};
	 			$herror{$keys}++; 
				$keys ="#droppable#";
	 			$herror{$keys}++; 
			}
