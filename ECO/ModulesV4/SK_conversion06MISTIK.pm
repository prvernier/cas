package ModulesV4::SK_conversion06MISTIK;


use Exporter;
our @ISA = qw(Exporter);
our @EXPORT = qw(&SKMISTIKinv_to_CAS );
...
sub NaturallyNonVegetated
{
	...
	{
		if (($NatNonVeg eq "CB"))	{ $NatNonVeg = "SL"; } # prev EX
		elsif (($NatNonVeg eq "SB"))	{ $NatNonVeg = "WS"; } # SA if adj to water
		elsif (($NatNonVeg eq "SL"))	{ $NatNonVeg = "SL"; } 
	}
...
sub NonForested
{
	...
	my $NonForVeg;
	my ($SP1_S,$CROWN_S,$SP1_H,$CROWN_H,$SMR,$L1CC,$L2CC,$L3CC) = @_;
	my $cond2 = 1;
	...
	if($sumCC < 10 && ($cond2 && $CROWN_S >= $CROWN_H))
	{
		$NonForVeg = $SP1_S;
	...
	elsif($sumCC < 10 && ($cond2 = 1 && $CROWN_S < $CROWN_H))
	{
		$NonForVeg = $SP1_H;
		if (isempty($NonForVeg))	{ $NonForVeg = MISSCODE; }
		elsif (($NonForVeg eq "FE"))	{ $NonForVeg = "HF"; } #TS turn into ST
		elsif (($NonForVeg eq "SA"))	{ $NonForVeg = "HF"; }
		elsif (($NonForVeg eq "BU"))	{ $NonForVeg = "HF"; }
		elsif (($NonForVeg eq "ST"))	{ $NonForVeg = "HF"; }
		elsif (($NonForVeg eq "NC"))	{ $NonForVeg = "HF"; }
		elsif (($NonForVeg eq "NM"))	{ $NonForVeg = "HF"; }
		elsif (($NonForVeg eq "CO"))	{ $NonForVeg = "HF"; }

		elsif (($NonForVeg eq "HE"))	{ $NonForVeg = "HE"; }
		elsif (($NonForVeg eq "GR"))	{ $NonForVeg = "HG"; }
		elsif (($NonForVeg eq "SE"))	{ $NonForVeg = "HG"; }
		elsif (($NonForVeg eq "FE"))	{ $NonForVeg = "BR"; }
		elsif (($NonForVeg eq "SM"))	{ $NonForVeg = "BR"; }
		elsif (($NonForVeg eq "LI"))	{ $NonForVeg = "BR"; }
	}
...
sub NonVegetatedAnth
{
	...
	if(!isempty($LANDUSE))
	{ 
		$NonVegAnth = uc($LANDUSE);
		$type = 1;
	}

	elsif(!isempty($NONFOR))
	{ 
		$NonVegAnth = uc($NONFOR);
		$type = 2;
	}

	if(isempty($NonVegAnth))	{ $NonVegAnth = MISSCODE;}
	elsif ($type == 2 ) { $NonVegAnth = "FA";}
	#elsif ($type == 2 && ($NonVegAnth eq "DI" || $NonVegAnth eq "FP") ) {$NonVegAnth = "LG";}
	
	elsif ($type == 1)
	{
		if (($NonVegAnth eq "GSOF"))	{ $NonVegAnth = "IN"; }
		elsif (($NonVegAnth eq "MUOU"))	{ $NonVegAnth = "IN"; }
		elsif (($NonVegAnth eq "MG"))	{ $NonVegAnth = "IN"; }
		elsif (($NonVegAnth eq "LMBY"))	{ $NonVegAnth = "IN"; }
		elsif (($NonVegAnth eq "SGDU"))	{ $NonVegAnth = "IN"; }
		elsif (($NonVegAnth eq "PEATC")){ $NonVegAnth = "IN"; }
		elsif (($NonVegAnth eq "BUPO"))	{ $NonVegAnth = "IN"; }


		elsif (($NonVegAnth eq "RWGU"))	{ $NonVegAnth = "FA"; }
		elsif (($NonVegAnth eq "VEGU"))	{ $NonVegAnth = "CL"; }
		elsif (($NonVegAnth eq "BUPG"))	{ $NonVegAnth = "SE"; }
		elsif (($NonVegAnth eq "CMTY"))	{ $NonVegAnth = "SE"; }
		elsif (($NonVegAnth eq "DMGU"))	{ $NonVegAnth = "OT"; }
		elsif (($NonVegAnth eq "FTOW"))	{ $NonVegAnth = "OT"; }
...
# SP1_1 is a hardwood  (TA,WB,BP,GA,MM,WE,BO) or absent and MOD1 is not in BO, CO, SI, CL
# SP1_1 is a sofwood (WS,BS,JP,BF,TL) and MOD1 is not BO,CO,SI,CL

sub UnprodForested {
	my $UnprodFor;
	my %UnprodForList = ("3000", 1, "3100", 1, "3200", 1, "3300", 1   );
	my @list = qw(BO CO SI CL);
	my @hlist =qw(TA WB BP GA MM WE BO);
	my @slist = qw(WS BS JP BF TL) ;
	...
	my $found_mod1 = grep($MOD1, @list);
	my $is_hard = grep($SP1, @hlist);
	my $is_soft = grep($SP1, @slist);
	...
	elsif (($UnprodFor  eq "3200"))	{ $UnprodFor  = "NP"; }
	elsif (($UnprodFor eq "3300"))	{$UnprodFor = "OM"; }  ##moved from other type
	elsif (($UnprodFor > 9999) || (($UnprodFor eq "3000") && (($MOD1 eq "BO") || ($MOD1 eq "CO") || ($MOD1 eq "SI")))  )	{$UnprodFor = "P"; } 
	elsif (($UnprodFor eq "3000") && ( $is_hard || is_empty($SP1sav) && !$found_mod1))	{ $UnprodFor = "SD"; }
	elsif (($UnprodFor eq "3000") && $is_soft  && !$found_mod1 )	{ $UnprodFor = "SC"; }
...
sub Disturbance
{
	...
	{ 
		...
		elsif (($ModCode  eq "WI") || ($ModCode eq "wi")) { $Mod="WF"; }
...
sub SKMISTIKinv_to_CAS 
{
	...
	my $cpt_ind; my $SpAss; my $SP1_S; my $CROWN_S; my $SP1_H; my $CROWN_H; my $landuse;
	##############################################
	...
	$csv->column_names ($csv->getline ($SKinv));
   	...
   	{	
		...
		$CrownClosure1 =  $row->{CROWN_1};
		$Height1 =  $row->{HEIGHT_1};
		$Origin1 =  $row->{ORIGIN_1};

		$CrownClosure2 =  $row->{CROWN_2};
		$Height2 =  $row->{HEIGHT_2};
		$Origin2 =  $row->{ORIGIN_2};

		$CrownClosure3 =  $row->{CROWN_3};
		$Height3 =  $row->{HEIGHT_3};
		$Origin3 =  $row->{ORIGIN_3};
		...
		$PHOTO_YEAR   = "1980"; # $row->{PHOTOYEAR}; #$row->{SYR};
		...
		{ 
			if (isempty($Sp11) &&  ($Height1 eq "0" ||isempty($Height1)))
			{
		...
		{ 
			if (isempty($Sp21) &&  ($Height2  eq "0" || isempty($Height2)))
			{
		...
		{ 
			if (isempty($Sp31) &&  ($Height3 eq "0" || isempty($Height3)))
			{
		...
		if($errspec == 1)
		{
			$keys="species layer 1".$SpeciesComp1."#original#$CAS_ID".$Sp11.",".$Sp12.",".$Sp13.",".$Sp14.",".$Sp15.",".$Sp16;
			$herror{$keys}++; 
		}
		...
		{
			$keys="Stotal perct !=100"."#$total#".$SpeciesComp1."#original#".$Sp11Per.",".$Sp12Per.",".$Sp13Per.",".$Sp14Per.",".$Sp15Per.",".$Sp16Per;
			$herror{$keys}++; ;
			$herror{$keys}++; 
			#$errspec=1;
		}
		...
		$Wetland = WetlandCodes_SFVI ($MoistReg, $CrownClosure1, $Height1,  $NonProd, $Sp11, $Sp12, $Sp1Per);
		...
		if($errspec == 1)
		{
			$keys="species layer 2".$SpeciesComp2."#original#".$Sp21.",".$Sp22.",".$Sp23.",".$Sp24.",".$Sp25.",".$Sp26;
			$herror{$keys}++; 
		}
		$total = $SpecsPerList[1] + $SpecsPerList[3]+ $SpecsPerList[5] +$SpecsPerList[7] +$SpecsPerList[9]+$SpecsPerList[11];
		...
		if($total != 100 && $total != 0 )
		{
			$keys="Stotal perct !=100"."#$total#".$SpeciesComp2."#original#".$Sp21Per.",".$Sp22Per.",".$Sp23Per.",".$Sp24Per.",".$Sp25Per.",".$Sp26Per;
			$herror{$keys}++; 
			#$errspec=1;
		}
		...
		if($errspec == 1)
		{
			$keys="species layer 3".$SpeciesComp3."#original#".$Sp31.",".$Sp32.",".$Sp33.",".$Sp34.",".$Sp35.",".$Sp36;
			$herror{$keys}++; 
		}
		$total = $SpecsPerList[1] + $SpecsPerList[3]+ $SpecsPerList[5] +$SpecsPerList[7] +$SpecsPerList[9]+$SpecsPerList[11];
		...
		if($total != 100 && $total != 0 )
		{
			$keys="Stotal perct !=100"."#$total#".$SpeciesComp3."#original#".$Sp31Per.",".$Sp32Per.",".$Sp33Per.",".$Sp34Per.",".$Sp35Per.",".$Sp36Per;
			$herror{$keys}++; 
			#$errspec=1;
		}
		...
		$OriginHigh1 = UpperOrigin($Origin1);
		$OriginLow1 = LowerOrigin($Origin1);
		if($OriginHigh1   eq ERRCODE   || $OriginLow1  eq ERRCODE ) 
		{ 
			if (isempty($Sp11) &&  ($Origin1 eq "0" ||isempty($Origin1)))
			{
				$OriginHigh1 = MISSCODE;
				$OriginLow1 = MISSCODE;
			}
			elsif ($Origin1 ne "0" && !empty_char{$Origin1})
			{
				$keys="Origin1"."#".$Origin1;
				$herror{$keys}++;
			}
		}
	 	if( ($OriginHigh1< $OriginLow1) || $OriginHigh1>2014 || ($OriginHigh1 <1700 && $OriginHigh1 >0) || ($OriginLow1 <1700 && $OriginLow1 >0) || $OriginLow1>2014) 
	 	{ 
			$keys="bounds for Origin1"."#".$Origin1;
			$herror{$keys}++;
		}
		...
		$OriginHigh2 = UpperOrigin($Origin2);
		$OriginLow2 = LowerOrigin($Origin2);
		if($OriginHigh2   eq ERRCODE   || $OriginLow2  eq ERRCODE ) 
		{ 
			if (isempty($Sp21) &&  ($Origin2 eq "0" ||isempty($Origin2)))
			{
				$OriginHigh2 = MISSCODE;
				$OriginLow2 = MISSCODE;
			}
			elsif ($Origin2 ne "0" && !empty_char{$Origin2})
			{
				$keys="Origin2"."#".$Origin2;
				$herror{$keys}++;
			}
		}
	 	if( ($OriginHigh2< $OriginLow2) || $OriginHigh2>2014 || ($OriginHigh2 <1700 && $OriginHigh2 >0) || ($OriginLow2 <1700 && $OriginLow2 >0) || $OriginLow2>2014) 
	 	{ 
			$keys="bounds for Origin2"."#".$Origin2;
			$herror{$keys}++;
		}
		...
		$OriginHigh3 = UpperOrigin($Origin3);
		$OriginLow3 = LowerOrigin($Origin3);
		if($OriginHigh3   eq ERRCODE   || $OriginLow3  eq ERRCODE ) 
		{ 
			if (isempty($Sp31) &&  ($Origin3 eq "0" ||isempty($Origin3)))
			{
				$OriginHigh3 = MISSCODE;
				$OriginLow3 = MISSCODE;
			}
			elsif ($Origin3 ne "0" && !empty_char{$Origin3})
			{
				$keys="Origin3"."#".$Origin3;
				$herror{$keys}++;
			}
		}
	 	if( ($OriginHigh3< $OriginLow3) || $OriginHigh3>2014 || ($OriginHigh3 <1700 && $OriginHigh3 >0) || ($OriginLow3 <1700 && $OriginLow3 >0) || $OriginLow3>2014) 
	 	{ 
			$keys="bounds for Origin3"."#".$Origin3;
			$herror{$keys}++;
		}
	    ...
	    # ===== Non-forested Land =====
		$NatNonVeg = NaturallyNonVegetated($NonProd, $Mod1, $CrownClosure1, $CrownClosure2, $CrownClosure3);
		$NonForVeg = NonForested($SP1_S,$CROWN_S,$SP1_H,$CROWN_H,$MoistReg,$CrownClosure1, $CrownClosure2, $CrownClosure3);
		$UnProdFor = UNDEF; #UnprodForested ($NonProd, $Sp11, $Mod1); #$NonProd);
		$NonVegAnth = NonVegetatedAnth($landuse, $NonProd);
		...
		if(($NatNonVeg  eq ERRCODE) && ($NonVegAnth eq ERRCODE) && ($NonForVeg eq ERRCODE)) 
		{
			...
			{ 
				$keys="Natnonveg"."#nonfor, mod, cc1, cc2,cc3==".$NonProd."--".$Mod1."--".$CrownClosure1."--".$CrownClosure2."--".$CrownClosure3;
				$herror{$keys}++;
			}
			if( ($NonVegAnth eq ERRCODE)) 
			{ 
				$keys="NonVegAnth"."#landuse, nonfor = ".$landuse."--".$NonProd;
				$herror{$keys}++;
			}
			...
			# 		$CROWN_H = "#BK#";
			# 	}
			# 	$keys="NonForVeg"."##SP1_S,CROWN_S,SP1_H,CROWN_H,SMR,CC1,CC2,CC3".$SP1_S."#".$CROWN_S."#".$SP1_H."#".$CROWN_H."#".$MoistReg."#".$CrownClosure1."#".$CrownClosure2."#".$CrownClosure3;
			# 	$herror{$keys}++;
			# }
		}
		...
		# my ($Sp31, @Spreste3) = split(",", $SpeciesComp3); 
		# my ($Sp21, @Spreste2) = split(",", $SpeciesComp2); 
		# my ($Sp11, @Spreste1) = split(",", $SpeciesComp1); 
		if (isempty($Sp11)) 
		{
		...
		$StandStructureCode = "S"; #StandStructure($row->{L1_LAYER_TYPE});
		...
		print CASCAS $CAS_Record . "\n";
	 	$nbpr=1;$$ncas++;$ncasprev++;
		    ...
		    {
		    	$LYR_Record1 = $CAS_ID . "," . $SMR  . "," . MISSCODE . ",1,1";
		   ...
		   elsif (!is_missing($NatNonVeg)  || !is_missing($NonForVeg) || !is_missing($NonVegAnth)) 
		    {
		      	...
		      	$NFL_Record1 = $CAS_ID. "," . $SMR  . "," . MISSCODE . ",1,1";
	   ...
	   #          $NFL_Record = $NFL_Record1 . "," . $NFL_Record2 . "," . $NFL_Record3;
		  #     	print CASNFL $NFL_Record . "\n";
				# $nbpr++;$$nnfl++;$nnflprev++;
		  # 	}
	 		elsif(isempty($NonProd))  # previous field was MLEVEL
	 		{
		    ...
		    if (!isempty($Mod1)) 
		    {
		   	...
		   	elsif ($NonProd eq "SIL" ||  $NonProd eq "EXP")  # previous field was MLEVEL
		   	{
		...
		if($nbpr ==1 || $nbpr ==0)
		{
			if(isempty($Sp11)  && isempty($row->{MOD1})  &&  isempty($NonProd) && $Wetland eq MISSCODE) 
			{	
				$keys = "WILL PROBABLY DROP THIS>>>code for wetland DRAIN+=";
			}
			elsif(isempty($Sp11)  && isempty($row->{MOD}) && isempty($NonProd)) 
			{
				$keys = "WILL DROP instead of wetland";
	 			$herror{$keys}++; 
			}
			elsif(isempty($Sp11)  && isempty($row->{MOD1})) #&& $row->{TYPE} eq "0"
			{
				$keys = "WILL DROP  TYPE == $row->{TYPE}";
	 			$herror{$keys}++; 
			}
			elsif(isempty($Sp11)) {
				$keys = "WILL DROP instead nfl and dist-TYPE="."-distcode=".$row->{MOD1};
	 			$herror{$keys}++; 
			}
			else 
			{
				$keys ="!!! record may be dropped#".$CAS_ID."bcse>>>specs=".$Sp11."-distcode=".$row->{MOD1}."-NPcode=".$NonProd."-wetcode=";
	 			$herror{$keys}++; 
				$keys ="#droppable#";
	 			$herror{$keys}++; 
			}
