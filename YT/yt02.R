...
sppList = read_csv("../CASFRI/translation/tables/lookup/yt02_yvi01_species.csv")

if (!exists("yt02")) {
    con = dbConnect(RPostgreSQL::PostgreSQL(), dbname="casfri50_pierrev", host="localhost", port=5432, user="postgres", password="1postgres")
    yt02 = st_read(con, query="SELECT * FROM rawfri.yt02 ORDER BY random() LIMIT 10000;")
    sink("YT/yt02.txt")
    cat("YT02 - FRI Attributes\n---------------------\n")
    for (i in friVars) {
        cat("\n\n", toupper(i),"\n\n", sep="")
        print(dfSummary(yt02[[i]], graph.col=FALSE))
    }
    sink()
    dbDisconnect(con)
}
...
dfSummary(yt02$nat_non_veg, graph.col=F)
dfSummary(yt02$non_for_anth, graph.col=F)
dfSummary(yt02$non_for_veg, graph.col=F)
dfSummary(yt02$nfl_soil_moist_reg, graph.col=F)

#rmarkdown::render("yt/yt02.Rmd", output_dir="yt02")
#browseURL("file://D:/PierreV/CAS/YT/yt02.html", browser="C:/Program Files (x86)/Google/Chrome/Application/chrome.exe")
#shiny::runApp("shiny")
